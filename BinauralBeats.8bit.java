...
class BinauralBeats {
		...
		*/
		...
		try{
			for(int samples = Math.round(sampleRate * duration), sampleOffset = 0; sampleOffset < samples; sampleOffset++){
				...
				///since encoding is 8-bit per sample, convert to byte
				writer.write((byte) (Math.round(127 * volume * sineWayPosition)));
			}
		}catch(IOException x) {
	...
	public static void makeFrequencySequenceFile(float[][] frequencySet, int sampleRate, float totalDuration, String fileName){
		...
		*/
		...
		try{
			...
			buffer.putInt(24);
			//data size.  Since we are using 8bits (1 byte) per sample, no need for further multipliers
			buffer.putInt(Math.round(sampleRate * totalDuration));
			...
			try{
				...
				for(int i = 0; i < frequencySet.length; i++){
					BinauralBeats.makeWave(writer, sampleRate, frequencySet[i][0], frequencySet[i][1], (float)0.5);
				}
			}catch(IOException x) {
				System.err.format("IOException: %s%n", x);
			}finally{
	...
	*/
	public static void makeBinaural(int centerFrequency, float[][] variations, String fileName){
		...
		BinauralBeats.makeFrequencySequenceFile(frequencySet1, 8000, totalDuration, "side1.au");
		BinauralBeats.makeFrequencySequenceFile(frequencySet2, 8000, totalDuration, "side2.au");
		BinauralBeats.combineSides(fileName, "side1.au", "side2.au", 8000, totalDuration);
	}
		...
		try{
			...
			try{
				...
				try{
					...
					for(int i = 24; i < side1Bytes.length; i++){
	...
	public static byte[] hexStringToByteArray(String s) {
		...
		for (int i = 0; i < len; i += 2) {
			data[i / 2] = (byte) ((Character.digit(s.charAt(i), 16) << 4)//convert firs character to digit, then move its bits 4 to the right, to make room for  the next hex character
